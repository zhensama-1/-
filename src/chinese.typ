...
#resume_desc("主修课程", "C语言程序设计99分，软件工程97分，数据结构96分，离散数学96分，高等数学（下）95分")
...
#resume_desc(
  "项目简介",
  "设计了一个场景重建系统，构建基于大语言模型的视觉智能体框架。系统通过生成并执行Blender程序实现 2D/3D 场景建模，结合多视角渲染与视觉验证模块进行迭代优化，实现文本到动态场景的自动生成与空间关系推理，探索视觉理解与逆图形建模结合的智能体方法。"
)
#resume_desc(
  "项目成果",
  "在 3D 编辑和 2D 设计任务中，该系统相比传统一次性生成模型分别提升了35.32%和117.17%同时，在30个极具挑战性的任务，涵盖相机调整、多步编辑和组合式绘图中，与SOTA对比实现了124.70%的性能突破 "
)
...
#resume_desc(
  "项目简介",
  "构建框架识别、论元边界检测与角色分类三阶段语义解析Pipeline，采用独立Span分类支持重叠论元，通过合法角色 Mask 降低无效预测。基于中文预训练模型设计目标词感知分类器，采用独立Span Sigmoid建模论元边界，支持重叠及嵌套论元识别；结合Frame Element Mask约束角色预测空间。设计伪标签方案，对外部实例执行跨数据集去重及置信度筛选。"
)
#resume_desc(
  "项目成果",
